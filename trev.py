...
#--------------------------------------------------------
	    elif msg.text in [frederick["trevor"]+"autokick:on"]:
		wait["AutoKick"] = True
		trev.sendText(msg.to,"AutoKick\n Active")
            elif msg.text in [frederick["trevor"]+"autokick:off"]:
	        wait["AutoKick"] = False
		trev.sendText(msg.to,"AutoKick\n inActive")
#--------------------------------------------------------
...
#--------------------------------------------------------
	    elif frederick["trevor"]+"mentionall" == msg.text.lower():
...
"▶️「"+frederick["trevor"]+"ssgoogle:\n"+
"▶️「"+frederick["trevor"]+"geturl」:\n"+
"▶️「"+frederick["trevor"]+"searchmovie: 」:\n"+
"▶️「"+frederick["trevor"]+"ssps: 」:\n"+
...
#--------------------------------------------------------
            ...
            elif frederick["trevor"]+"ssps: " in msg.text.lower():
                        sep = msg.text.lower().replace(frederick["trevor"]+"geturl: ","")                                                                                                                            
                        with requests.session() as web:
                                r = web.get("http://rahandiapi.herokuapp.com/sswebAPI?key=betakey&link=https://play.google.com/store/search?q=".format(urllib2.quote(sep)))
                                data = r.text
                                data = json.loads(data)
                                trev.sendImageWithURL(msg.to, data["result"])
            elif frederick["trevor"]+"ssgoogle: " in msg.text.lower():
                        sep = msg.text.lower().replace(frederick["trevor"]+"ssgoogle: ","")                                                                                                                            
                        with requests.session() as web:
                                r = web.get("http://rahandiapi.herokuapp.com/sswebAPI?key=betakey&link=https://www.google.com/".format(urllib2.quote(sep)))
                                data = r.text
                                data = json.loads(data)
                                trev.sendImageWithURL(msg.to, data["result"])
            elif frederick["trevor"]+'music: ' in msg.text.lower():
                try:
                    songname = msg.text.lower().replace(frederick["trevor"]+'music: ','')
